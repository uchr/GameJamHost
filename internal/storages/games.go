...
func (st *storage) CreateGame(ctx context.Context, game gamejams.Game) error {
	_, err := st.db.Exec(ctx, "INSERT INTO games (game_jam_id, user_id, title, content, cover_image, screenshot_images, url, build, is_banned) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
		game.GameJamID, game.UserID, game.Title, game.Content, game.CoverImageURL, pq.StringArray(game.ScreenshotURLs), game.URL, game.Build, game.IsBanned)
...
func (st *storage) GetGame(ctx context.Context, jamID int, gameURL string) (*gamejams.Game, error) {
	...
	err := row.Scan(&game.ID, &game.GameJamID, &game.UserID, &game.Title, &game.Content, &game.CoverImageURL, (*pq.StringArray)(&game.ScreenshotURLs), &game.URL, &game.Build, &game.IsBanned)
	if err != nil {
...
func (st *storage) GetGames(ctx context.Context, jamID int) ([]gamejams.Game, error) {
	...
	for rows.Next() {
		var game gamejams.Game
		err = rows.Scan(&game.ID, &game.GameJamID, &game.UserID, &game.Title, &game.Content, &game.CoverImageURL, (*pq.StringArray)(&game.ScreenshotURLs), &game.URL, &game.Build, &game.IsBanned)
		if err != nil {
			return nil, err
		}
