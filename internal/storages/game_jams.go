...
func (st *storage) CreateJam(ctx context.Context, jam gamejam.GameJam) error {
	_, err := st.db.Exec(ctx, "INSERT INTO game_jams (name, url, content, start_date, end_date, voting_end_date, hide_results, hide_submissions) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
		jam.Title, jam.URL, jam.Content, jam.StartDate, jam.EndDate, jam.VotingEndDate, jam.HideResults, jam.HideSubmissions)
	if err != nil {
		return err
	}
	return nil
...
func (st *storage) GetJam(ctx context.Context, jamID int) (*gamejam.GameJam, error) {
	...
	err := row.Scan(&gameJam.ID, &gameJam.Title, &gameJam.URL, &gameJam.Content, &gameJam.StartDate, &gameJam.EndDate, &gameJam.VotingEndDate, &gameJam.HideResults, &gameJam.HideSubmissions)
	if err != nil {
		return nil, err
	}
...
func (st *storage) UpdateJam(ctx context.Context, jam gamejam.GameJam) error {
	_, err := st.db.Exec(ctx, "UPDATE game_jams SET name = $1, url = $2, content = $3, start_date = $4, end_date = $5, voting_end_date = $6, hide_results = $7, hide_submissions = $8 WHERE game_jam_id = $9",
		jam.Title, jam.URL, jam.Content, jam.StartDate, jam.EndDate, jam.VotingEndDate, jam.HideResults, jam.HideSubmissions, jam.ID)
	if err != nil {
		return err
	}
	return nil
...
func (st *storage) GetJams(ctx context.Context) ([]gamejam.GameJam, error) {
	...
	for rows.Next() {
		var gameJam gamejam.GameJam
		err = rows.Scan(&gameJam.ID, &gameJam.Title, &gameJam.URL, &gameJam.Content, &gameJam.StartDate, &gameJam.EndDate, &gameJam.VotingEndDate, &gameJam.HideResults, &gameJam.HideSubmissions)
		if err != nil {
			return nil, err
		}
